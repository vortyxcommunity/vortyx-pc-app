-- MASTER PROJECT FIX v2 (Run this in Supabase SQL Editor)
...
-- Check if tables exist before altering replica identity (safety)
DO $$ BEGIN
    ALTER TABLE IF EXISTS public.dm_calls REPLICA IDENTITY FULL;
    ALTER TABLE IF EXISTS public.server_voice_states REPLICA IDENTITY FULL;
...
BEGIN
    ...
    END IF;
    -- DM Calls
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'dm_calls') AND NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'dm_calls') THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.dm_calls;
    END IF;
    -- Voice States
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'server_voice_states') AND NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'server_voice_states') THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.server_voice_states;
    END IF;
    -- Messages
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'messages') AND NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'messages') THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.messages;
    END IF;
    -- DM Messages
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'dm_messages') AND NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'dm_messages') THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.dm_messages;
    END IF;
     -- Reactions
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'message_reactions') AND NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'message_reactions') THEN
